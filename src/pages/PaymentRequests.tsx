...
import {
  ...
  AlertTriangle, ArrowRight, Send, Shield, Star
} from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import { fr } from "date-fns/locale";
import { SendPaymentRequestModal } from "@/components/payments/SendPaymentRequestModal";
import { RatingModal } from "@/components/trades/RatingModal";
...
const PaymentRequests = () => {
  ...
  }>({ open: false, tradeId: "", ratedUserId: "", ratedUserName: "" });
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success") {
      ...
      toast({ title: "Paiement réussi !", description: "La transaction a été enregistrée." });
      if (transactionId) {
  ...
  const handlePay = async (transactionId: string) => {
    ...
    try {
      ...
      toast({ title: "Erreur", description: error?.message || "Impossible de procéder au paiement.", variant: "destructive" });
    } finally {
      setPayingId(null);
    }
  };
  ...
  const handleDecline = async (transactionId: string) => {
    toast({ title: "Demande refusée", description: "La demande de paiement a été ignorée." });
  };

  const handleRate = (tx: Transaction) => {
    const isSeller = tx.seller_id === user?.id;
    const otherUserId = isSeller ? (tx.buyer_id || "") : tx.seller_id;
    const otherProfile = profiles.get(otherUserId);
    const otherName = otherProfile?.full_name || otherProfile?.username || "Utilisateur";

    // Use the transaction ID as trade reference for rating
    setRatingModal({
      open: true,
      tradeId: tx.id,
      ratedUserId: otherUserId,
      ratedUserName: otherName,
    });
  };

  const isExpired = (tx: Transaction) => tx.expires_at && new Date(tx.expires_at) < new Date();

  const getStatusInfo = (tx: Transaction) => {
    if (tx.status === "completed" && tx.escrow_status === "pending_escrow") return { label: "Fonds sécurisés (48h)", icon: Clock, className: "text-blue-500" };
    if (tx.status === "completed" && tx.escrow_status === "released") return { label: "Fonds versés", icon: CheckCircle, className: "text-green-500" };
  ...
  const receivedRequests = transactions.filter((t) => t.buyer_id === user?.id && t.method === "remote" && t.status === "pending" && !isExpired(t));
  const sentRequests = transactions.filter((t) => t.seller_id === user?.id && t.method === "remote");
  const allTransactions = transactions.filter((t) => t.method !== "remote" || t.status !== "pending");
  ...
  return (
    <MainLayout showSearch={false}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          @{seller?.username || seller?.full_name || "vendeur"}
                        </p>
                      </div>
                      <p className="text-2xl font-bold text-destructive">-{tx.amount}€</p>
                    </div>
                    <div className="flex items-start gap-2 p-3 bg-destructive/10 rounded-xl text-sm">
                      <AlertTriangle className="h-4 w-4 text-destructive mt-0.5 shrink-0" />
                      <p className="text-muted-foreground">Ce paiement est définitif.</p>
                    </div>
                    <p className="text-xs text-muted-foreground text-center">
                      Expire {tx.expires_at ? formatDistanceToNow(new Date(tx.expires_at), { addSuffix: true, locale: fr }) : "bientôt"}
                    </p>
                    <div className="flex gap-2">
                      <Button variant="outline" className="flex-1" onClick={() => handleDecline(tx.id)}>
                        <XCircle className="h-4 w-4 mr-2" />Refuser
                      </Button>
                      <Button variant="gameswap" className="flex-1" disabled={payingId === tx.id} onClick={() => handlePay(tx.id)}>
                        {payingId === tx.id ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <ArrowRight className="h-4 w-4 mr-2" />}
                        Payer
                      </Button>
                    </div>
                  </div>
              ...
              {sentRequests.map((tx) => {
                ...
                return (
                  <div key={tx.id} className="bg-card rounded-2xl border border-border p-4 flex items-center gap-3">
                    {game?.image_url && <img src={game.image_url} alt="" className="w-12 h-12 rounded-xl object-cover" />}
                    <div className="flex-1 min-w-0">
                      <p className="font-bold truncate">{game?.title || "Jeu"}</p>
                      <p className="text-sm text-muted-foreground">@{buyer?.username || buyer?.full_name || "acheteur"}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-emerald-500">+{tx.amount}€</p>
          ...
          {allTransactions.length === 0 && receivedRequests.length === 0 && sentRequests.length === 0 ? (
            ...
            <div className="space-y-2">
              {allTransactions.map((tx) => {
                ...
                const isSeller = tx.seller_id === user?.id;
                const otherUserId = isSeller ? tx.buyer_id : tx.seller_id;
                const otherProfile = otherUserId ? profiles.get(otherUserId) : null;

                return (
                  <div key={tx.id} className="bg-card rounded-xl border border-border p-3 space-y-2">
                    <div className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm truncate">{game?.title || "Jeu"}</p>
                        <p className="text-xs text-muted-foreground">
                          @{otherProfile?.username || otherProfile?.full_name || "utilisateur"} ·{" "}
                          {formatDistanceToNow(new Date(tx.created_at), { addSuffix: true, locale: fr })}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className={`font-bold text-sm ${isSeller ? "text-emerald-500" : "text-destructive"}`}>
                          {isSeller ? "+" : "-"}{tx.amount}€
                        </p>
                        <p className={`text-xs flex items-center gap-1 justify-end ${statusInfo.className}`}>
                          <StatusIcon className="h-3 w-3" />{statusInfo.label}
                    ...
                    {tx.method === "cash" && tx.status === "completed" && (
                      <div className="flex items-center gap-2 p-2 bg-yellow-500/10 rounded-lg text-xs">
                        ...
                        <span className="text-yellow-600 dark:text-yellow-400">Non protégée</span>
                      </div>
                    )}

                    {/* Rate button for completed transactions */}
                    {tx.status === "completed" && tx.method !== "cash" && (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="w-full text-xs"
                        onClick={() => handleRate(tx)}
                      >
      ...
      <SendPaymentRequestModal open={sendModalOpen} onOpenChange={setSendModalOpen} onSuccess={fetchData} />
